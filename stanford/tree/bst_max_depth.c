...
int 
bst_max_depth_count(struct bst_node *root)
{
	int ltree_depth;
	int rtree_depth;
	...
	if (root->bst_link[0] == NULL && root->bst_link[1] == NULL)
		return 1;

    ltree_depth = 1;
	rtree_depth = 1;

	ltree_depth += bst_max_depth_count(root->bst_link[0]);
	rtree_depth += bst_max_depth_count(root->bst_link[1]);
	return ltree_depth > rtree_depth ? ltree_depth : rtree_depth; 
}

int 
bst_max_depth(struct bst_table *t)
{

	if (t == NULL || t->bst_root == NULL)
		return 0;

	return bst_max_depth_count(t->bst_root);
}
