#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include <errno.h>
#include <assert.h>

/* bst compare function */
typedef int (*bst_cmp_func_t)(void *, void *);

/* bst node */
struct bst_node {
    void *bst_data;
    struct bst_node *bst_link[2];
};

/* bst */
struct bst_table {
    struct bst_node *bst_root;
    bst_cmp_func_t bst_cmp;
    void *bst_cmp_param;
    unsigned int bst_count;
};

void
bst_print(const struct bst_node *node)
{
    if (node == NULL)
        return;

    fprintf(stderr, "%d:", *(int *)node->bst_data);
    if (node->bst_link[0] != NULL || node->bst_link[1] != NULL) {
        fprintf(stderr, "(");
        bst_print(node->bst_link[0]);
        if (node->bst_link[1] != NULL) {
            fprintf(stderr, ",");
            bst_print(node->bst_link[1]);
        }
        fprintf(stderr, ")");
    }
}

struct bst_node *
get_node(void *bst_data)
{
    struct bst_node *n;

    if ((n = (struct bst_node *) malloc(sizeof(struct bst_node))) == NULL) {
        fprintf(stderr, "ERROR : malloc : %s\n", strerror(errno));
        return 0;
    }
    n->bst_data = bst_data;
    n->bst_link[0] = n->bst_link[1] = NULL;
    return n;
}

struct bst_table *
bst_create(bst_cmp_func_t bst_cmp)
{
    struct bst_table *t;

    assert(bst_cmp != NULL);

    if ((t = (struct bst_table *) malloc(sizeof(struct bst_table))) == NULL) {
        fprintf(stderr, "ERROR : malloc : %s\n", strerror(errno));
        return NULL;
    }
    t->bst_root = NULL;
    t->bst_cmp = bst_cmp;
    t->bst_cmp_param = NULL;
    t->bst_count = 0;

    return t;
}

void
bst_insert(struct bst_table *t, void *item)
{
    int dir;
    int cmp;
    struct bst_node *n;
    struct bst_node *p;
    struct bst_node *q;

    assert(t != NULL && item != NULL);

    for (q = NULL, p = t->bst_root; p != NULL; q = p, p = p->bst_link[dir]) {
        cmp = t->bst_cmp(item, p->bst_data);
        if (cmp == 0) {
            return;
        }
        dir = cmp > 0;
    }

    n = get_node(item);
    if (n == NULL)
        return;

    if (q == NULL) {
        t->bst_root = n;
    } else {
        q->bst_link[dir] = n;
    }
    t->bst_count++;
}

void
bst_delete(struct bst_table *t)
{
	struct bst_node *p;
	struct bst_node *q;

	if (t == NULL || t->bst_root == NULL)
		return;

	for (p = t->bst_root; p != NULL; p = q) {
		if (p->bst_link[0] != NULL) {
			q = p->bst_link[0];
			p->bst_link[0] = q->bst_link[1];
			q->bst_link[1] = p;
		} else {
			q = p->bst_link[1];
			free(p);
		}
	}
}

bool
bst_tree_compare(struct bst_node *r1, struct bst_node *r2)
{
	bool t1;
	bool t2;

	if (r1 == NULL && r2 == NULL)
		return true;

	if ((r1 != NULL && r2 == NULL) || (r1 == NULL && r2 != NULL))
		return false;

	if ((*(int*)r1->bst_data) != (*(int*)r2->bst_data))
		return false;

	t1 = false;
	t2 = false;

	t1 = bst_tree_compare(r1->bst_link[0], r2->bst_link[0]);
	if (t1 == true)
		t2 = bst_tree_compare(r1->bst_link[1], r2->bst_link[1]);

	return t1 == true && t2 == true ? true : false;
}

int bst_int_cmp(void *a, void *b)
{
    int ret;

    assert(a != NULL && b != NULL);

    ret = 0;
    if (*(int *)a > *(int *)b) {
        ret = 1;
    } else if (*(int *)a < *(int *)b) {
        ret = -1;
    }

    return ret;
}

void
test_case_1(void)
{
    struct bst_table *t1;
    struct bst_table *t2;

    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	t2 = bst_create(bst_int_cmp);
	bst_print(t1->bst_root);
	putchar('\n');
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);
}

void
test_case_2(void)
{
	int arr[1] = {50}; 

    struct bst_table *t1;
    struct bst_table *t2;

    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	bst_insert(t1, &arr[0]);
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);

    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	bst_insert(t2, &arr[0]);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);

}

void
test_case_3_1(void)
{
	int a[2] = {50}; 
	int b[2] = {50}; 
    struct bst_table *t1;
    struct bst_table *t2;

    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	bst_insert(t1, &a[0]);
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	bst_insert(t2, &b[0]);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);

	a[0] = 50; b[0] = 60;
    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	bst_insert(t1, &a[0]);
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	bst_insert(t2, &b[0]);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);

	a[0] = 50; a[1] = 60;
	b[0] = 50; b[1] = 60;
    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	bst_insert(t1, &a[0]);
	bst_insert(t1, &a[1]);
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	bst_insert(t2, &b[0]);
	bst_insert(t2, &b[1]);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);

	a[0] = 50; a[1] = 40;
	b[0] = 50; b[1] = 60;
    t1 = NULL;
    t2 = NULL;
	
	t1 = bst_create(bst_int_cmp);
	bst_insert(t1, &a[0]);
	bst_insert(t1, &a[1]);
	bst_print(t1->bst_root);
	putchar('\n');

	t2 = bst_create(bst_int_cmp);
	bst_insert(t2, &b[0]);
	bst_insert(t2, &b[1]);
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);
	
}

void
test_case_3_3(void)
{
	int i;
	int a[14] = {50, 20, 15, 10, 3, 2, 30, 100, 75, 125, 80, 90, 110, 130}; 
	int b[14] = {50, 20, 15, 10, 3, 2, 30, 100, 75, 125, 80, 90, 110, 130}; 
    struct bst_table *t1;
    struct bst_table *t2;

    t1 = NULL;
    t2 = NULL;
	t1 = bst_create(bst_int_cmp);
	for (i = 0; i < 14; i++) {
		bst_insert(t1, &a[i]);
	}
	bst_print(t1->bst_root);
	putchar('\n');
	t2 = bst_create(bst_int_cmp);
	for (i = 0; i < 14; i++) {
		bst_insert(t2, &b[i]);
	}
	bst_print(t2->bst_root);
	putchar('\n');
	printf("%sSAME\n", bst_tree_compare(t1->bst_root, t2->bst_root) == true ?
			"" : "NOT ");
	bst_delete(t1);
	bst_delete(t2);
    free(t1);
    free(t2);
}


void
test_case_3(void)
{
/*	test_case_3_1();
	test_case_3_2();*/
	test_case_3_3();
}

int main(void)
{
	//test_case_1();
	//test_case_2();
	test_case_3();

    return 0;
}
