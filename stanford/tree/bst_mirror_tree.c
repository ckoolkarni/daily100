...
void
bst_make_mirror(struct bst_node *r)
{
	struct bst_node *t;

	if ((r == NULL) ||
		(r->bst_link[0] == NULL && r->bst_link[1] == NULL)) {
		return;
	}
	
	bst_make_mirror(r->bst_link[0]);
	bst_make_mirror(r->bst_link[1]);

	t = r->bst_link[0];
	r->bst_link[0] = r->bst_link[1];
	r->bst_link[1] = t;
}

void 
bst_mirror_tree(struct bst_table *t)
{

	if (t == NULL)
		return;
	
	bst_make_mirror(t->bst_root);
}
