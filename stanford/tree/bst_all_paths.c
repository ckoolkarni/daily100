...
void
bst_print_path(struct bst_node *r, struct bst_node *stack[], int stop)
{
	int i;
	if (r == NULL) {
		return;
	...
	if (r->bst_link[0] == NULL && r->bst_link[1] == NULL) {
		fprintf(stderr, "PATH : ");
		for (i = 0; i < stop; i++)
			fprintf(stderr, "%d ", *(int *)stack[i]->bst_data);
		fprintf(stderr, "\n");
	} else {
			bst_print_path(r->bst_link[0], stack, stop);
			bst_print_path(r->bst_link[1], stack, stop);
	}
}

void 
bst_print_all_path(struct bst_table *t)
{
    struct bst_node *stack[BST_MAX_DEPTH];
	if (t == NULL)
		return;
		
	bst_print_path(t->bst_root, stack, 0);
}
...
{
	int i;
	int arr[14] = {50, 20, 15, 10, 3, 2, 30, 100, 75, 125, 80, 90, 110, 130}; 
    struct bst_table *t;
