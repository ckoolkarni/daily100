...
void
bst_inorder(struct bst_node *r)
{
	if (r == NULL)
		return;

	bst_inorder(r->bst_link[0]);
	fprintf(stderr, "%d ", *(int *)r->bst_data);
	bst_inorder(r->bst_link[1]);
}

void 
bst_inorder_print(struct bst_table *t)
{
	if (t == NULL)
		return;

	bst_inorder(t->bst_root);
}
